...
FROM (SELECT c_custkey,
      ...
      GROUP BY c_custkey) AS c_orders (c_custkey,
                                       c_count)
GROUP BY c_count
ORDER BY custdist DESC,
         c_count DESC;

-- TPC-H Query 13 (aggregation pushdown in subquery, only correct if LEFT OUTER JOIN is JOIN)
SELECT c_count,
       count(*) AS custdist
FROM (
     WITH aggregation_pushdown AS (
         SELECT o_custkey, count(o_orderkey) AS o_count
         FROM orders
         WHERE o_comment NOT LIKE '%special%requests%'
         GROUP BY o_custkey
     )  SELECT c_custkey, o_count
     FROM customer
              LEFT OUTER JOIN aggregation_pushdown ON c_custkey = o_custkey
...
-- ORDER keys are a subset (or equal) to the GROUP BY keys.
-- TPC-H Query 19
