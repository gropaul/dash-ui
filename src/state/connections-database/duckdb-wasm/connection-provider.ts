...
const DUCKDB_TABLE_NAME = 'main.duckdb';
...
export async function clearOPFS(): Promise<void> {
    ...
    const walPath = `${DUCKDB_TABLE_NAME}.wal`;

    const rootHandle = await navigator.storage.getDirectory();
    const dbHandle = await rootHandle.removeEntry(DUCKDB_TABLE_NAME);
